...
from utils.multi_label.metric import MetricReport
from paddle.io import BatchSampler, DataLoader, DistributedBatchSampler
from utils.multi_label.utils import evaluate, preprocess_function, read_local_dataset
...
parser.add_argument("--dataset_dir", default="./data/divorce/", type=str, help="Local dataset directory should include train.txt, dev.txt and label.txt")
parser.add_argument("--save_dir", default="./checkpoint/divorce/", type=str, help="The output directory where the model checkpoints will be written.")
...
def parse_arg(args, args_dict):
    # 属性替换,用于retraining的参数传递
    if "device" in args_dict:
        args.device = args_dict["device"]
    if "data_dir" in args_dict:
        args.data_dir = args_dict["data_dir"]
    if "save_dir" in args_dict:
        args.save_dir = args_dict["save_dir"]
    if "max_seq_length" in args_dict:
        args.max_seq_length = args_dict["max_seq_length"]
    if "model_name" in args_dict:
    ...
    if "early_stop" in args_dict:
        args.early_stop = args_dict["early_stop"]
    if "early_stop_nums" in args_dict:
        args.early_stop_nums = args_dict["early_stop_nums"]
    if "logging_steps" in args_dict:
        args.logging_steps = args_dict["logging_steps"]
    if "weight_decay" in args_dict:
        args.weight_decay = args_dict["weight_decay"]
    if "warmup" in args_dict:
        args.warmup = args_dict["warmup"]
    if "warmup_steps" in args_dict:
        args.warmup_steps = args_dict["warmup_steps"]
    if "init_from_ckpt" in args_dict:
        args.init_from_ckpt = args_dict["init_from_ckpt"]
    if "seed" in args_dict:
        args.seed = args_dict["seed"]
    if "train_file" in args_dict:
        args.train_file = args_dict["train_file"]
    if "dev_file" in args_dict:
        args.dev_file = args_dict["dev_file"]
    if "label_file" in args_dict:
        args.label_file = args_dict["label_file"]
...
def train(**args_dict):
    ...
    label_list = {}
    with open(
        os.path.join(args.dataset_dir, args.label_file), "r", encoding="utf-8"
    ) as f:
    ...
    train_ds = load_dataset(
        read_local_dataset,
        path=os.path.join(args.dataset_dir, args.train_file),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        early_stop_count += 1
        micro_f1_score, macro_f1_score = evaluate(
